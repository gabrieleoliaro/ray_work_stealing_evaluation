...
output_path="output/${timestamp}_work-stealing"
mkdir -p $output_path
	...
	for max_tasks_in_flight in ${max_tasks_in_flight_vals[@]}; do
		...
		for task_duration in ${individual_task_durations[@]}; do
			...
			for (( i=0; i<$ntrials; ++i)); do
				timeout 200s python variable_length_tasks.py -s ${total_sequential_duration} -i ${task_duration} -c ${ncpus} -p ${max_tasks_in_flight} -w ${work_stealing} -o ${output_file} -d 0
				ray stop --force
			done
		done
	done
...
python plot_work_stealing.py -t ${ntrials} -p "${max_tasks_in_flight_vals[*]}" -s ${total_sequential_duration} -i "${individual_task_durations[*]}" -o ${output_path}